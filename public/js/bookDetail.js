...
function openStickyModal(deleteFlg) {
  if (!modalFlg) {
    ...
    $('#modal').css('display', 'block'); //モーダル以外を暗くする処理
...
function closeModal() {
  //モーダルを隠す処理
  $('#modal').find(".message-box").css('display', 'none');
  $('#modal').css('display', 'none'); //暗くした要素を元にもどす処理
...
$('.sticky-regist-button').on('click', function () {
  ...
  var bookPage = modalBox.find(".modal-book-page").children('input').val();
  var stickyTitle = modalBox.find(".modal-sticky_title").children('input').val();
  var stickyMemo = modalBox.find(".modal-sticky_content").children('textarea').val();
  ...
  var formattedMemo = splitText.join('<br>');
  var urlString = "";
